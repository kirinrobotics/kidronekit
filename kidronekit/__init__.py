# DroneAPI module
"""
This is the API Reference for the kidronekit-Python API.
...
.. code:: python
    ...
    from kidronekit import connect
...
from kidronekit.util import ErrprinterHandler


class APIException(Exception):
    """
    Base class for kidronekit related exceptions.
...
class VehicleMode(object):
    ...
    The recommended flight modes for *kidronekit-Python* apps depend on the vehicle type:
...
class Vehicle(HasObservers):
    ...
    def __init__(self, handler):
        ...
        self._logger = logging.getLogger(__name__)  # Logger for kidronekit
        ...
        def listener(self, name, m):
            ...
            self._climb = m.climb
            self.notify_attribute_listeners('climb', self.climb)
            self._groundspeed = m.groundspeed
            self.notify_attribute_listeners('groundspeed', self.groundspeed)
            self._alt = m.alt
            self.notify_attribute_listeners('alt', self.alt)
            self._throttle = m.groundspeed
            self.notify_attribute_listeners('throttle', self.throttle)
    ...
    def last_heartbeat(self):
        ...
        .. note::
            ...
            parameter passed to the :py:func:`connect() <kidronekit.connect>` function.
    ...
    def location(self):
        ...
        * :py:attr:`global_frame <kidronekit.Locations.global_frame>` (:py:class:`LocationGlobal`)
        * :py:attr:`global_relative_frame <kidronekit.Locations.global_relative_frame>` (:py:class:`LocationGlobalRelative`)
        * :py:attr:`local_frame <kidronekit.Locations.local_frame>` (:py:class:`LocationLocal`)
    ...
    def climb(self):
        """
        Vertical Speed - Current climb rate m/s (``double``).
        """
        return self._climb
    ...
    def throttle(self):
        """
        Current throttle setting (0 to 100 %). (``uint16_t``).
        """
    ...
    def ekf_ok(self):
        ...
        """
        # legacy check for kidronekit-python for solo
    ...
    def channels(self):
        ...
        via :py:attr:`Vehicle.channels.override <kidronekit.Channels.overrides>`.
    ...
    def send_mavlink(self, message):
        ...
        The :py:func:`message_factory <kidronekit.Vehicle.message_factory>` is used to create messages in the appropriate format.

        For more information see the guide topic: :ref:`guided_mode_how_to_send_commands`.

        :param message: A ``MAVLink_message`` instance, created using :py:func:`message_factory <kidronekit.Vehicle.message_factory>`.
    ...
    def message_factory(self):
        ...
        The message can then be sent using :py:func:`send_mavlink(message) <kidronekit.Vehicle.send_mavlink>`.
        ...
        id (just set to zero) as kidronekit will automatically update messages with the correct ID for the connected
        vehicle before sending.
        The ``target_component`` should be set to 0 (broadcast) unless the message is to specific component.
        CRC fields and sequence numbers (if defined in the message type) are automatically set by kidronekit and can also
...
class Command(mavutil.mavlink.MAVLink_mission_item_message):
    ...
    :param target_system: This can be set to any value
        (kidronekit changes the value to the MAVLink ID of the connected vehicle before the command is sent).
...
class CommandSequence(object):
    ...
    def clear(self):
        ...
        if home:
            self._vehicle._wploader.add(home, comment='Added by kidronekit')
    ...
    def add(self, cmd):
        ...
        self._vehicle._wploader.add(cmd, comment='Added by kidronekit')
        self._vehicle._wpts_dirty = True
...
def connect(ip,
    ...
    :param bool use_native: Use precompiled MAVLink parser.
        ...
        .. note::
            ...
            autopilot, configure the ``kidronekit`` and ``autopilot`` loggers using the Python ``logging`` module.


    :returns: A connected vehicle of the type defined in ``vehicle_class`` (a superclass of :py:class:`Vehicle`).
    """

    from kidronekit.mavlink import MAVConnection
